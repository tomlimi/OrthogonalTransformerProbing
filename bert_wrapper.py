...
#
#     _MAX_SENTENCE_LEN = 512
#
#     def __init__(self, language,  layer_index, size="base", casing="uncased", with_cls=False, threads=1, batch_size=16):
...
#
#         self.model_path = "/net/projects/bert/models/{}-{}-{}".format(language, size, casing)
#         if not os.path.exists(self.model_path):
#             raise ValueError("The requested Bert model combination {}-{}-{} does not exist".format(language, size, casing))
#
#         # Initialize the Bert model
#         self._tokenizer = FullTokenizer(vocab_file="{}/vocab.txt".format(self.model_path), do_lower_case=self._uncased)
#
#         self.bert_config = BertConfig.from_json_file("{}/bert_config.json".format(self.model_path))
#
#     def model_fn(self, features, mode):
#         model = BertModel(config=self.bert_config, is_training=False, input_ids=features["input_ids"],
...
#         tf.train.init_from_checkpoint("{}/bert_model.ckpt".format(self.model_path), assignment_map)
#         all_layers = model.get_all_encoder_layers()
#
#         predictions = {"unique_id": features["unique_ids"]}
#
#         return all_layers[self._layer_index]
#
#
#
...
#         #         embedding_dim = result["layer_output_{}".format(self._layer_index)].shape[1]
#         #
...
#         #         if self._with_cls:
#         #             current_embeddings[0] += result["layer_output_{}".format(self._layer_index)][0]
#         #
#         #     for i, token_id in enumerate(features[feature_index].token_ids):
#         #         if token_id >= 0:
#         #             current_embeddings[token_id + self._with_cls] += result["layer_output_{}".format(self._layer_index)][i] / (token_subwords[sentence][token_id])
#         #
...
class BertWrapper:
    ...
    def __init__(self, language, size="base", casing="uncased", layer_indices=[-1, -2, -3, -4], with_cls=False,
                 threads=1, batch_size=16):
        """Construct BertWrapper instance
        ...
        def model_fn(features, labels, mode, params):
            model = BertModel(config=bert_config, is_training=False, input_ids=features["input_ids"],
                              input_mask=features["input_mask"], token_type_ids=features["input_type_ids"],
                              use_one_hot_embeddings=False)
            assert mode == tf.estimator.ModeKeys.PREDICT

            tvars = tf.trainable_variables()
            (assignment_map, initialized_variable_names) = get_assignment_map_from_checkpoint(tvars,
                                                                                              "{}/bert_model.ckpt".format(
                                                                                                  model_path))
        ...
        self._estimator = tf.estimator.Estimator(
            model_fn=model_fn,
            config=tf.estimator.RunConfig(session_config=tf.ConfigProto(inter_op_parallelism_threads=threads,
                                                                        intra_op_parallelism_threads=threads)))

    def bert_embeddings(self, sentences, max_sent_len=128):
        ...
        InputFeatures = collections.namedtuple("InputFeatures",
                                               "unique_id sentence subwords token_ids input_ids input_mask input_type_ids")
        ...
        def input_fn(params):
            dataset = tf.data.Dataset.from_generator(
                ...
                {"unique_ids": [], "input_ids": [self._MAX_SENTENCE_LEN], "input_mask": [self._MAX_SENTENCE_LEN],
                 "input_type_ids": [self._MAX_SENTENCE_LEN]})
            dataset = dataset.batch(batch_size=self._batch_size, drop_remainder=False)
            return dataset

        embedding_dim = None

        embeddings = tf.zeros((len(sentences), max_sent_len), dtype=tf.float32)

        for s_idx, result in enumerate(self._estimator.predict(input_fn, yield_single_examples=True)):
            if embedding_dim is None:
            ...
            sentence = features[feature_index].sentence
            if feature_index == 0 or features[feature_index].sentence != features[feature_index - 1].sentence:
                # current_embeddings = np.zeros((len(sentences[sentence]) + self._with_cls, embedding_dim),
                #                               dtype=np.float32)
                if self._with_cls:
                    for layer_index in self._layer_indices:
                        embeddings[s_idx, 0] += result["layer_output_{}".format(layer_index)][0] / len(
                            self._layer_indices)

            for i, token_id in enumerate(features[feature_index].token_ids):
                if token_id >= 0:
                    for layer_index in self._layer_indices:
                        embeddings[s_idx, token_id + self._with_cls] += \
                            result["layer_output_{}".format(layer_index)][i] / (
                                    token_subwords[sentence][token_id] * len(self._layer_indices))

            # Yield results if the whole sentence was processed
            # if feature_index + 1 == len(features) or features[feature_index].sentence != features[
            #     feature_index + 1].sentence:
            #     yield current_embeddings
