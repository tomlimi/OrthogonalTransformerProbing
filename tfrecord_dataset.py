...
class Dataset:
    ...
    class EmbeddedData:
        ...
        def calc_embeddings(self, wordpieces, segments, token_len):
            ...
            embeddings = [tf.map_fn(lambda x: tf.math.unsorted_segment_mean(x[0], x[1], x[2]),
                                    (emb, segments, max_token_len), dtype=tf.float32) for emb in
                          embeddings]
        ...
        def serialize_example(embeddings, token_len, task_target_mask):
            ...
            feature.update({f'layer_{idx}': Dataset._bytes_feature(tf.io.serialize_tensor(layer_embeddings))
                        for idx, layer_embeddings in enumerate(embeddings)})
            
            for task, (target, mask) in task_target_mask.items():
                feature.update({f'target_{task}': Dataset._bytes_feature(tf.io.serialize_tensor(target)),
                                f'mask_{task}': Dataset._bytes_feature(tf.io.serialize_tensor(mask))})
        ...
        def write_tfrecord(self):
            ...
            with tf.io.TFRecordWriter(filename) as writer:
                for target, mask, wordpieces, segments, token_len in tqdm(self.dependency_data.target_and_mask(), desc="Embedding computation"):


                    # print("target:\n", target[:15,:15])
                    # print("mask:\n", mask[:15, :15])
                    # print("segments: ", segments[:15])
                    # print("wordpieces: ", wordpieces[:15])
                    # print("token len: ", token_len)

                    #task_target_mask = {task: (target, mask) for task, (target, mask) in zip(self.tasks, task_target_mask)}
                    task_target_mask = {self.tasks[0]: (target, mask)}


                    embeddings = self.calc_embeddings(wordpieces, segments, token_len)
